...
plt.title("Pie Chart for Japan University Hosts",y=2.8)
...
plt.figure(figsize=(15,12))
plt.title("Pie Chart for UK University Hosts",y=2.8)
...
plt.figure(figsize=(15,12))
plt.title("Pie Chart for US University Hosts",y=2.8)
...
Japan_date_hour_count_pandas = Japan_date_hour_count.toPandas()
Japan_heatmap_DF = pd.pivot_table(Japan_date_hour_count_pandas, values='count',index='hour', columns='date')
...
UK_date_hour_count_pandas = UK_date_hour_count.toPandas()
UK_heatmap_DF = pd.pivot_table(UK_date_hour_count_pandas, values='count',index='hour', columns='date')
...
US_date_hour_count_pandas = US_date_hour_count.toPandas()
US_heatmap_DF = pd.pivot_table(US_date_hour_count_pandas, values='count',index='hour', columns='date')

## Ploting heatmap for Japan
print("Ploting heatmap for Japan")
plt.figure(figsize=(12,10))
plt.imshow(Japan_heatmap_DF, cmap='hot')
plt.xticks(np.arange(int(max(Japan_date_hour_count_pandas['date']))))
plt.yticks(np.arange(int(max(Japan_date_hour_count_pandas['hour']))))
...
plt.xticks(np.arange(int(max(UK_date_hour_count_pandas['date']))))
plt.yticks(np.arange(int(max(UK_date_hour_count_pandas['hour']))))
plt.xlabel("Date", fontsize=13)
plt.ylabel("Hour", fontsize=13)
...
plt.xticks(np.arange(int(max(US_date_hour_count_pandas['date']))))
